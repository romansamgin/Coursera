...
public:
    ...
    WriteAccess GetWriteAccess(size_t num, const K& key) const {
        return { lock_guard<mutex>(buckets[GetIndex(key)].mtx), 
            buckets[GetIndex(key)].storage[key] };
    }


    ReadAccess GetReadAccess(size_t num, const K& key) const {
        return { lock_guard<mutex>(buckets[GetIndex(key)].mtx), 
            buckets[GetIndex(key)].storage.at(key) };
    }


    WriteAccess operator[](const K& key) {
        return GetWriteAccess(GetIndex(key), key);
    }

    ReadAccess At(const K& key) const {
        if (buckets[GetIndex(key)].storage.find(key) == buckets[GetIndex(key)].storage.end()) {
            throw std::out_of_range("out_of_range");
        }
        else {
            return GetReadAccess(GetIndex(key), key);
        }    
    }

    bool Has(const K& key) const {
        auto& bucket = buckets[GetIndex(key)];
        lock_guard g(bucket.mtx);
        return bucket.storage.count(key) > 0;
    }
